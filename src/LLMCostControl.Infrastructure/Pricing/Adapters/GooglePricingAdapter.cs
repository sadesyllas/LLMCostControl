...
/// <summary>
/// Pricing adapter for Google. Fetches current pricing from a configurable URL
/// that serves the canonical pricing file format (§8.5). On failure, falls back
/// to persisted DB values with a staleness signal.
/// </summary>
public sealed class GooglePricingAdapter : PricingAdapterBase
{
    private readonly HttpClient _httpClient;
    private readonly string _sourceUrl;

    /// <summary>The provider this adapter handles.</summary>
    public override Provider Provider => Provider.Google;

    /// <summary>
    /// Creates the adapter.
    /// </summary>
    /// <param name="httpClient">The HTTP client used for live fetching.</param>
    /// <param name="sourceUrl">The URL serving the canonical pricing file for Google.</param>
    /// <param name="repository">The repository for persisted fallback reads.</param>
    public GooglePricingAdapter(HttpClient httpClient, string sourceUrl, ModelPricingRepository repository)
        : base(repository)
    {
    ...
    /// canonical pricing file, filtering to Google entries only.
    /// </summary>
    protected override async Task<IReadOnlyCollection<ModelPricing>> FetchLiveAsync(CancellationToken ct)
    {
        ...
        if (!result.IsValid)
        {
            throw new InvalidOperationException(
                $"Google pricing source returned an invalid file: {string.Join("; ", result.Errors)}");
        }

        return result.Entries
            .Where(e => e.Provider == Provider.Google)
            .ToList();
    }
}
